...
VERSION='0.1'
LANGAUGE='en'
ENDPOINT='places/bbox' # This challenge requires to get all accommodations inside a square between a defined longitude and latitude
FORMAT='json'
KINDS='accomodations'
...
request_parameters = {"kinds": KINDS,
...
print(f"{r.status_code}") if r.status_code == 200 else print(f"Something went wrong, "
                                                             f"API returned {r.status_code} response.")

print(f"{len(r.json())} rows entries extracted correctly.") if len(r.json()) == LIMIT else print(f"Something went wrong"
                                                                                                 f", expected {LIMIT} "
                                                                                                 f"entries but got "
                                                                                                 f"{len(r.json())}.")
